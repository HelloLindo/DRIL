'''
    ...
    Deep Q-Networks Playing Pong Game
...
def get_args():
    ...
    parser.add_argument('--save-dir', type=str, default="./data/" + time.strftime("%Y%m%d", time.localtime()) + "/DQN")
    # Device
    ...
    for episode in range(n_episodes):
        ...
        if episode % 10 == 0 or episode == n_episodes-1:
            print('Total steps: {} \t Episode: {}/{} \t Total reward: {}'.format(args.steps_done, episode, t,
                                                                                 total_reward))
            np.save(args.save_dir + "/episode_pong_dqn", episode_list)
            np.save(args.save_dir + "/reward_pong_dqn", reward_list)
            np.save(args.save_dir + "/q_value_pong_dqn", q_value_list)
...
def pong_DQN(args=None):
    ...
    SAVE_PATH = args.save_dir + "/pong_model_dqn.pt"
